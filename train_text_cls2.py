...
# python train_text_cls2.py --order 1 --learner oml2 --model gpt2 --max_length 1024
...
if __name__ == '__main__':
    ...
    # Run ID
    TASK_TRIAD = str(args.order)
    DATETIME = datetime.today().strftime('%Y%m%dT%H%M%S')
    RUN_TYPE = args.learner + args.model
    ...
    fileHandler = logging.FileHandler(f'{model_dir}/{RUN_ID}.log')
    ...
    learner.training(dataset_order_mapping[args.order], **vars(args)) ## Change the loaded datasets to the dataset order instead!!!!
#     learner.save_model(os.path.join(model_dir, model_file_name))       # save only once??
...
#     learner.testing(dataset_order_mapping[args.order], **vars(args))
